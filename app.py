...
def transform_view():
    ...
    file_contents = json.load(open(request_file.filename, encoding='utf-8-sig'))
    result = transform(file_contents)

    outputfile ="result_"+ request_file.filename
    # outputpath = RESULT_FOLDER + '/' +outputfile
    # with open(outputpath, 'w') as data_file:
    #     json.dump(result, data_file, indent=4, sort_keys=True)
    # response = send_from_directory(directory = RESULT_FOLDER, filename = outputfile,as_attachment=True)
